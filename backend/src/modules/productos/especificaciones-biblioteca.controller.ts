...
export async function crear(req: Request, res: Response) {
  try { res.status(201).json(await crearEspecificacionBiblioteca(Number(req.context!.empresaId), req.auth?.userId ?? null, req.body)); } catch (e) { responderError(res, e); }
}
export async function actualizar(req: Request, res: Response) {
  try { const row = await actualizarEspecificacionBiblioteca(Number(req.params.id), Number(req.context!.empresaId), req.auth?.userId ?? null, req.body); if (!row) return res.status(404).json({ error: 'Especificación no encontrada' }); res.json(row); } catch (e) { responderError(res, e); }
}
export async function reordenar(req: Request, res: Response) {
  try { await reordenarEspecificacionesBiblioteca(Number(req.context!.empresaId), req.auth?.userId ?? null, Array.isArray(req.body?.ids) ? req.body.ids : []); res.json({ ok: true }); } catch (e) { responderError(res, e); }
}
export async function baja(req: Request, res: Response) {
  try { const row = await cambiarBajaEspecificacionBiblioteca(Number(req.params.id), Number(req.context!.empresaId), req.auth?.userId ?? null, req.body?.baja !== false); if (!row) return res.status(404).json({ error: 'Especificación no encontrada' }); res.json(row); } catch (e) { responderError(res, e); }
}
