...
export async function insertarProductoRepository(data: any) {
  ...
  const cols = ['empresa_id', ...campos.filter(campo => campo in data)];
  const values = [empresaId, ...campos.filter(campo => campo in data).map(campo => data[campo])];
...
export async function updateProductoRepository(id: number, data: any) {
  ...
  const set = campos
    .filter(campo => campo in data)
    .map((campo, idx) => `${campo} = $${idx + 1}`)
    .join(', ');
  const values = campos.filter(campo => campo in data).map(campo => data[campo]);
