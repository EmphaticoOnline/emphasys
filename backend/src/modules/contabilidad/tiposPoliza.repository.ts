...
// contabilidad.tipos_poliza.id/empresa_id son bigint/bigserial: node-pg los
// regresa como string aunque TipoPoliza los declare number. Sin este cast,
// cualquier comparación estricta contra un number real (ej. el frontend
// buscando el tipo configurado en Configuración → Tipos automáticos) falla
// en silencio.
function mapearTipoPoliza(row: any): TipoPoliza {
  return { ...row, id: Number(row.id), empresa_id: Number(row.empresa_id) };
}
...
export async function listarTiposPoliza(empresaId: number, opts: { soloActivos?: boolean } = {}): Promise<TipoPoliza[]> {
  ...
  return rows.map(mapearTipoPoliza);
}
...
export async function obtenerTipoPolizaPorId(id: number, empresaId: number): Promise<TipoPoliza | null> {
  ...
  return rows[0] ? mapearTipoPoliza(rows[0]) : null;
}
...
export async function crearTipoPoliza(empresaId: number, input: TipoPolizaInput): Promise<TipoPoliza> {
  ...
  return mapearTipoPoliza(rows[0]);
}
