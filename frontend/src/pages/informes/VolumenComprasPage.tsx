...
export type VolumenComprasPageConfig = {
  ...
  fetchFn: (params: VolumenContactoParams) => Promise<VolumenContactoResult>;
  buildExportUrl: (params: VolumenContactoParams, formato: 'excel' | 'pdf') => string;
};
...
function buildColumnsResumen(contactoLabel: string): GridColDef<ContactoVolumen>[] {
  ...
  const columns: GridColDef<ContactoVolumen>[] = [
    ...
    },
    {
      ...
      align: 'right',
      headerAlign: 'right',
      renderCell: (p: GridRenderCellParams<ContactoVolumen, number>) => (
...
function DetalleTable({
  ...
  const facturasPorContacto = useMemo(() => {
    ...
    for (const f of facturas) {
      if (!m.has(f.contacto_id)) m.set(f.contacto_id, []);
      m.get(f.contacto_id)!.push(f);
    }
    return m;
  }, [facturas]);
  ...
  return (
    <Stack spacing={2}>
      {contactos.map((p) => {
        const facs = facturasPorContacto.get(p.contacto_id) ?? [];
        return (
          <Box key={p.contacto_id}>
...
export default function VolumenComprasPage({ config }: { config: VolumenComprasPageConfig }) {
  ...
  const [resultado, setResultado] = useState<VolumenContactoResult | null>(null);
  ...
  useEffect(() => {
    ...
    const params: VolumenContactoParams = {
      fecha_inicio: fechaInicio,
      fecha_fin: fechaFin,
      contacto_id: proveedor?.id ?? null,
  ...
  const totalComprado = contactos.reduce((s, p) => s + p.total_comprado, 0);
  const totalFacturas = contactos.reduce((s, p) => s + p.cantidad_facturas, 0);
          ...
          <>
            <Box sx={{ height: 520 }}>
              <DataGrid
                ...
                getRowId={(row) => (row as ContactoVolumen).contacto_id}
