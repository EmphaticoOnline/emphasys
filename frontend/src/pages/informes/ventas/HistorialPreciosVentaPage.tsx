...
import {
  fetchHistorialPreciosVenta,
  buildHistorialPreciosVentaExportUrl,
...
export default function HistorialPreciosVentaPage() {
  ...
  const [opcionesCliente, setOpcionesCliente]   = useState<ContactoOpcion[]>([]);
  ...
  const buscarClientes = useCallback((input: string) => {
    ...
    clienteTimer.current = setTimeout(async () => {
      ...
      try {
        const qs = new URLSearchParams({ limit: '40', tipos: 'cliente' });
        if (input.trim()) qs.set('search', input.trim());
  ...
  useEffect(() => {
    ...
    fetchRef.current = setTimeout(async () => {
      ...
      setError(null);
      try {
        const data = await fetchHistorialPreciosVenta(buildParams());
        setResultado(data);
      } catch (err) {
  ...
  }, [fechaInicio, fechaFin, producto, cliente]);

  const handleExportar = async (formato: 'excel' | 'pdf') => {
    if (exportando) return;
    setExportando(formato);
    try {
      const url = buildHistorialPreciosVentaExportUrl(buildParams(), formato);
      ...
      const filename = match?.[1] ?? `historial-precios-venta.${ext}`;
  ...
  return (
    <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
      ...
      <Stack direction="row" alignItems="center" spacing={1}>
        ...
        <Typography color="text.disabled">/</Typography>
        <Typography variant="body2" fontWeight={600}>Historial de Precios de Venta</Typography>
      </Stack>

      <Box>
        <Typography variant="h6" fontWeight={700}>Historial de Precios de Venta</Typography>
        <Typography variant="caption" color="text.secondary">
          Evolución de precios unitarios en facturas de venta. Compara clientes y analiza variaciones de precio.
        </Typography>
      </Box>
      ...
      <Paper sx={{ px: 2, py: 1, position: 'relative', overflow: 'hidden' }}>
        ...
        <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <Autocomplete<ContactoOpcion>
            ...
            onOpen={() => { if (!opcionesCliente.length) buscarClientes(''); }}
            ...
            renderInput={(inputProps) => (
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              <TextField {...(inputProps as any)} label="Cliente (todos)" size="small" />
            )}
          />
      ...
      {resumen && (
        <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap' }}>
          {[
            { label: 'Último precio',    valor: formatMXN(resumen.ultimo_costo),   color: '#1d2f68', bold: true },
            { label: 'Precio mínimo',    valor: formatMXN(resumen.costo_min),      color: 'success.main', bold: false },
            { label: 'Precio máximo',    valor: formatMXN(resumen.costo_max),      color: 'error.main', bold: false },
      ...
      <Paper sx={{ p: 1.5 }}>
          ...
          <Box sx={{ height: 520 }}>
            <DataGrid
              ...
              localeText={{
                ...
                noRowsLabel: 'No hay ventas en el período indicado',
              }}
        ...
        {resultado && !loading && rows.length === 0 && (
          <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
            No hay facturas de venta en el período {formatFecha(fechaInicio)} – {formatFecha(fechaFin)}.
          </Typography>
        )}
      </Paper>
