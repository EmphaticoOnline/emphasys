...
export type EstadoDocumentoLoteVenta = 'contabilizada' | 'omitida_ya_contabilizada' | 'error';
...
export async function contabilizarFacturasVentaLote(payload: {
  fecha_desde: string;
  fecha_hasta: string;
  tipo_poliza_id?: number;
  agrupacion: AgrupacionLoteVenta;
}): Promise<ResultadoLoteVenta> {
