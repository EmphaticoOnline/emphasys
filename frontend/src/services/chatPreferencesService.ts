...
// Trae AMBAS preferencias en una sola llamada (viven en el mismo objeto JSON
// de la pantalla 'leads-chat'). Importante: el endpoint PUT reemplaza todo el
// objeto `preferencias`, no lo mezcla — por eso siempre se lee/escribe el par
// completo junto, nunca un campo suelto, para no pisar el otro al guardar.
export async function fetchChatSoundPreferences(perfilDispositivo: ChatDeviceProfile): Promise<ChatSoundPreferences> {
  try {
    ...
    return { sonidoActivado: true, tonoMensajes: DEFAULT_NOTIFICATION_TONE };
  }
}

// Siempre recibe el par completo (sonidoActivado + tonoMensajes): quien llama
// (LeadsPage.tsx) debe pasar el valor vigente del campo que NO está
// cambiando, para no perderlo en el reemplazo.
